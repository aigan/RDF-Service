...
#  $Id: serv1.pl,v 1.2 2001/04/03 14:18:20 aigan Exp $  -*-perl-*-
...
sub get_session
{
	...
	$session_res->set_abbrev(
	    {
		fn  	     => NS_LD.'Person/first_name',
		ln  	     => NS_LD.'Person/last_name',
		email 	     => NS_LD.'Person/email_address',
		icq  	     => NS_LD.'Person/icq_number',
...
sub do_initiate_db
{
	...
	my $cProperty 	= $s->get( NS_RDF.'Property' );
	my $cLiteral 	= $s->get( NS_RDFS.'Literal' );
	
	my $nsPerson	= NS_LD.'Person/';

	# Class Person
    	my $cPerson	= $model->get($nsPerson.'Person')->set(
	  [$cClass],
	{
	    label      => \"Person",
	});
	...
	# Property first_name
    $model->get($nsPerson.'first_name')->set(
	  [$cProperty],
	{
	    domain	=> $cPerson,
	    range	=> $cLiteral,
	...
	# Property last_name
    $model->get($nsPerson.'last_name')->set(
	  [$cProperty],
	{
	    domain	=> $cPerson,
	    range	=> $cLiteral,
	...
	# Property icq_number
    $model->get($nsPerson.'icq_number')->set(
	  [$cProperty],
	{
	    domain	=> $cPerson,
	    range	=> $cLiteral,
	...
	# Property email_adress
    $model->get($nsPerson.'email_address')->set(
	  [$cProperty],
	{
	    domain	=> $cPerson,
	    range	=> $cLiteral,
	...
	# Property comment
    $model->get($nsPerson.'comment')->set(
	  [$cProperty],
	{
	    domain	=> $cPerson,
	    range	=> $cLiteral,
...
sub do_state
{
	...
	if( $dir eq 'pred-lit' )
	{
	    set_pred_lit( $focus, $args[0], $val, $solid );
	}
	elsif( $dir eq 'pred' )
	{
	    set_pred( $focus, $args[0], $val, $solid );
	}
	elsif( $dir eq 'subj_pred' )
	{
	    set_subj_pred( $focus, @args[0,1], $val, $solid );
	}
	elsif( $dir eq 'li' )
	{
	    set_li( $focus, $args[0], $val, $solid );
	}
	elsif( $dir eq 'obj-lit' )
	{
	    set_obj_lit( $focus, $args[0], $val, $solid );
	}
	elsif( $dir eq 'pred_obj-lit' )
	{
	    set_pred_obj_lit( $focus, @args[0,1], $val, $solid );
	}
	elsif( $dir eq 'res-lit' )
	{
	    set_res_lit( $focus, $args[0], $val, $solid );
	}
    }
...
sub set_pred_lit
{
    my( $self, $pred, $vals, $solid ) = @_;

    debug_start("set_pred_lit", '+', $self);
...
sub set_pred
{
    ...
    debug_end("set_pred", '+', $self);
}

sub set_subj_pred
{
    my( $self, $subj, $pred, $vals, $solid ) = @_;

    debug_start("set_subj_pred", '+', $self);
    ...
    debug_end("set_subj_pred", '+', $self);
}

sub set_li
{
    my( $self, $cont, $vals, $solid ) = @_;

    debug_start("set_li", '+', $self);
    debug "  (@$vals)\n";
    ...
    debug_end("set_li", '+', $self);
}

sub set_res_lit
{
    my( $self, $res, $vals, $solid ) = @_;

    debug_start("set_res_lit", '+', $self);

    $res = $self->get($res);

    my $lit_str	= $vals->[0];

	$res->set_literal( \$lit_str );

    debug_end("set_res_lit", '+', $self);
}
