...
#  $Id: serv1.pl,v 1.8 2001/04/11 16:51:19 aigan Exp $  -*-perl-*-
...
sub get_session
{
	...
	$session_res->set_abbrev(
	    {
		fn       => NS_LD.'Property#first_name',
		ln       => NS_LD.'Property#last_name',
		agent    => NS_LS.'agent',
...
sub do_person_add
{
    ...
    my $types = [NS_LD.'Class#Person'];
    my $props =
    {
	NS_LD.'Property#first_name' => [\$r_fn],
	NS_LD.'Property#last_name'  => [\$r_ln],
    };
...
#    $self->[NODE][REV_SUBJ_ALL] == 2 or $self->init_rev_subjs;
    $s->set_props({NS_LS.'agent' => [$person]}, 2);
    $s->store;
