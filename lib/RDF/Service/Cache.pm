#  $Id: Cache.pm,v 1.30 2001/04/11 16:51:20 aigan Exp $  -*-perl-*-
...
use vars qw( $uri2id $id2uri $ids @EXPORT_OK %EXPORT_TAGS $create_cnt
	     $create_time $prefixlist $node %fc $dynamic_model );
...
sub debug_start
{
    ...
    $fc{$call}++; # Function counter.  Finds commonly used functions
    my $msg = ' 'x($Level-1);
#    my $msg = '|  'x($Level-1);
    $msg .= "/-- $no $call       $fc{$call}\n";
    warn $msg;
    ...
    my $node = $res->[NODE];
    if( $res and $node->[RUNLEVEL] )
    {
	my $ids = $node->[IDS];
	debug( $node->[URISTR]." IDS $node->[IDS]\n", 1);
	debug( "TYPE_ALL       $node->[TYPE_ALL]\n", 3) if $node->[TYPE_ALL];
	debug( "TYPE_SOLID     $node->[TYPE_SOLID]\n", 3) if $node->[TYPE_SOLID];
	debug( "REV_TYPE_ALL   $node->[REV_TYPE_ALL]\n", 3) if $node->[REV_TYPE_ALL];
	debug( "REV_TYPE_SOLID $node->[REV_TYPE_SOLID]\n", 3) if $node->[REV_TYPE_SOLID];
	debug( "REV_SUBJ_ALL   $node->[REV_SUBJ_ALL]\n", 3) if $node->[REV_SUBJ_ALL];
	debug( "REV_SUBJ_SOLID $node->[REV_SUBJ_SOLID]\n", 3) if $node->[REV_SUBJ_SOLID];
	debug( "REV_PRED_ALL   $node->[REV_PRED_ALL]\n", 3) if $node->[REV_PRED_ALL];
	debug( "REV_PRED_SOLID $node->[REV_PRED_SOLID]\n", 3) if $node->[REV_PRED_SOLID];
	debug( "REV_OBJ_ALL    $node->[REV_OBJ_ALL]\n", 3) if $node->[REV_OBJ_ALL];
	debug( "REV_OBJ_SOLID  $node->[REV_OBJ_SOLID]\n", 3) if $node->[REV_OBJ_SOLID];
	debug( "SOLID          $node->[SOLID]\n", 3) if $node->[SOLID];
	debug( "LOCAL          $node->[LOCAL]\n", 3) if $node->[LOCAL];
	debug( "W: $res->[WMODEL][NODE][URISTR] ".
	       "IDS $res->[WMODEL][NODE][IDS]\n", 2);
	confess "IDS mismatch" if $ids ne $res->[WMODEL][NODE][IDS];
	if( $node->[MODEL] )
	{
	    debug( "M: $node->[MODEL][URISTR] ".
		     "IDS $node->[MODEL][IDS]\n", 2);
	    confess "IDS mismatch" if $ids ne $node->[MODEL][IDS];
	}

	validate_context( $res ) if $DEBUG >= 7;
    }
}
...
sub validate_context
{
    ...
    # during the initialization of the resource, the existing types
    # will be set up before the new types is asigned.  During the
    # period in between, the resource will not have the right type.
    ...
    unless( ref $context->[SESSION] eq 'RDF::Service::Resource' )
    {
	confess "Session $context->[SESSION] is not a resource";
    }
...
sub validate_node
{
    ...
    unless( $uri =~ /^http/ )
    {
	confess "The URI '$uri' may realy be a literal";
    }
	...
	if( $model->[URISTR] ne NS_LS.'The_Base_Model' )
	{
...
sub validate_prop
{
	...
	# It seems that the pred #label is somewhere being created
	# with an empty list as value.  That was once meant to
	# indicate that we have looked for that property and didn't
	# found any.  ...  We will accept empty lists.

# 	unless( @{$node->[$rel]{$pred_id}} )
# 	{
# 	    my $msg = $node->[URISTR] ."->[$rel]{$uri} ".
# 	      "should not be empty";
# 	    confess $msg;
# 	}
